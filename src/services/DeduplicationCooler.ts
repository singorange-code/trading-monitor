...
import { ClassifiedAlert, AlertLevel } from '../types';
...
export class DeduplicationCooler {
  private cooldownMap: Map<string, CooldownEntry> = new Map();
  private readonly cooldownPeriod: number = 30 * 60 * 1000; // 30分钟
  private notificationStats: Map<string, number> = new Map();
  private adaptiveThresholds: Map<string, number> = new Map();

  async shouldNotify(alert: ClassifiedAlert): Promise<boolean> {
    try {
      ...
      if (existing && now < existing.cooldownUntil) {
        ...
        if (this.isLevelUpgrade(existing.level, alert.level)) {
          logger.info(`Level upgrade detected, bypassing cooldown for ${key}`);
          this.updateCooldownEntry(key, alert, now);
          return true;
        }
        
        logger.debug(`Alert ${key} is in cooldown until ${existing.cooldownUntil}`);
        return false;
      }
  ...
  private generateKey(alert: ClassifiedAlert): string {
    return `${alert.opportunity.symbol}_${alert.opportunity.strategy}_${alert.level}`;
  }

  private isLevelUpgrade(existingLevel: AlertLevel, newLevel: AlertLevel): boolean {
    const levelOrder = {
      [AlertLevel.WATCH]: 1,
      [AlertLevel.READY]: 2,
      [AlertLevel.FIRED]: 3
  ...
  private updateCooldownEntry(key: string, alert: ClassifiedAlert, now: Date): void {
    ...
    logger.debug(`Updated cooldown for ${key} until ${cooldownUntil}`);
  }

  private async isAboveAdaptiveThreshold(alert: ClassifiedAlert): Promise<boolean> {
  ...
  private getBaseThreshold(level: AlertLevel): number {
    switch (level) {
  ...
  private updateNotificationStats(symbol: string): void {
  ...
  private adjustAdaptiveThreshold(symbol: string): void {
    ...
    if (notificationCount > 50) {
      ...
      logger.info(`Adaptive threshold increased for ${symbol}: ${newThreshold}`);
    }
  }

  async mergeSimilarSignals(alerts: ClassifiedAlert[]): Promise<ClassifiedAlert[]> {
    if (alerts.length <= 1) return alerts;

    const merged: ClassifiedAlert[] = [];
    const processed = new Set<string>();

    for (const alert of alerts) {
      if (processed.has(alert.opportunityId)) continue;

      const similar = alerts.filter(a => 
        !processed.has(a.opportunityId) &&
      ...
      if (similar.length > 1) {
        // 选择最高级别的信号
        const bestAlert = similar.reduce((best, current) => 
          this.compareLevels(current.level, best.level) > 0 ? current : best
        );
  ...
  private compareLevels(level1: AlertLevel, level2: AlertLevel): number {
    const levelOrder = {
      [AlertLevel.WATCH]: 1,
      [AlertLevel.READY]: 2,
      [AlertLevel.FIRED]: 3
  ...
  async cleanupExpiredEntries(): Promise<void> {
    ...
    if (cleanedCount > 0) {
      logger.debug(`Cleaned up ${cleanedCount} expired cooldown entries`);
    }
  }

  getCooldownStatus(): { active: number; total: number } {
    const now = new Date();
