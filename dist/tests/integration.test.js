"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.IntegrationTest = void 0;
const DataCollector_1 = require("../services/DataCollector");
const OpportunityEngine_1 = require("../services/OpportunityEngine");
const RiskAssessor_1 = require("../services/RiskAssessor");
const NotificationService_1 = require("../services/NotificationService");
const logger_1 = require("../utils/logger");
/**
 * 集成测试 - 验证完整的监控流程
 */
class IntegrationTest {
    dataCollector;
    opportunityEngine;
    riskAssessor;
    notificationService;
    constructor() {
        this.dataCollector = new DataCollector_1.DataCollector();
        this.opportunityEngine = new OpportunityEngine_1.OpportunityEngine();
        this.riskAssessor = new RiskAssessor_1.RiskAssessor();
        this.notificationService = new NotificationService_1.NotificationService();
    }
    async runFullPipelineTest() {
        try {
            logger_1.logger.info('Starting full pipeline integration test...');
            // 1. 测试数据采集
            const testSymbol = 'BTCUSDT';
            const marketData = await this.dataCollector.collectMarketData(testSymbol);
            if (!marketData || marketData.price <= 0) {
                throw new Error('Data collection failed');
            }
            logger_1.logger.info(`✓ Data collection test passed: ${testSymbol} price = ${marketData.price}`);
            // 2. 测试机会识别
            const opportunities = await this.opportunityEngine.analyzeMarket(marketData);
            logger_1.logger.info(`✓ Opportunity analysis test passed: found ${opportunities.length} opportunities`);
            // 3. 测试风险评估
            if (opportunities.length > 0) {
                const riskAssessment = await this.riskAssessor.assessRisk(opportunities[0]);
                logger_1.logger.info(`✓ Risk assessment test passed: risk score = ${riskAssessment.riskScore}`);
            }
            // 4. 测试邮件服务连接
            const emailTest = await this.notificationService.testConnection();
            if (!emailTest) {
                logger_1.logger.warn('⚠ Email service test failed, but continuing...');
            }
            else {
                logger_1.logger.info('✓ Email service test passed');
            }
            logger_1.logger.info('✅ All integration tests passed!');
            return true;
        }
        catch (error) {
            logger_1.logger.error('❌ Integration test failed:', error);
            return false;
        }
    }
    async runPerformanceTest() {
        try {
            logger_1.logger.info('Starting performance test...');
            const testSymbols = ['BTCUSDT', 'ETHUSDT', 'BNBUSDT'];
            const startTime = Date.now();
            // 并行测试多个币种的数据采集
            const promises = testSymbols.map(symbol => this.dataCollector.collectMarketData(symbol));
            const results = await Promise.all(promises);
            const duration = Date.now() - startTime;
            const successCount = results.filter(r => r && r.price > 0).length;
            const avgTimePerSymbol = duration / testSymbols.length;
            logger_1.logger.info(`Performance test results:`, {
                totalSymbols: testSymbols.length,
                successCount,
                totalTime: duration,
                avgTimePerSymbol,
                successRate: (successCount / testSymbols.length) * 100
            });
            // 性能要求：平均每个币种不超过2秒
            if (avgTimePerSymbol > 2000) {
                logger_1.logger.warn('⚠ Performance test warning: average time per symbol exceeds 2s');
            }
            else {
                logger_1.logger.info('✓ Performance test passed');
            }
            return successCount === testSymbols.length;
        }
        catch (error) {
            logger_1.logger.error('❌ Performance test failed:', error);
            return false;
        }
    }
    async runHealthCheckTest() {
        try {
            logger_1.logger.info('Starting health check test...');
            // 测试数据采集器健康状态
            const dataCollectorHealth = await this.dataCollector.healthCheck();
            if (dataCollectorHealth.status !== 'healthy') {
                throw new Error(`Data collector unhealthy: ${dataCollectorHealth.status}`);
            }
            logger_1.logger.info('✓ Health check test passed');
            return true;
        }
        catch (error) {
            logger_1.logger.error('❌ Health check test failed:', error);
            return false;
        }
    }
    async runAllTests() {
        const tests = [
            { name: 'Full Pipeline', test: () => this.runFullPipelineTest() },
            { name: 'Performance', test: () => this.runPerformanceTest() },
            { name: 'Health Check', test: () => this.runHealthCheckTest() }
        ];
        const results = [];
        let passed = 0;
        let failed = 0;
        for (const { name, test } of tests) {
            try {
                logger_1.logger.info(`\n🧪 Running ${name} test...`);
                const result = await test();
                if (result) {
                    passed++;
                    results.push({ name, status: 'PASSED', result });
                    logger_1.logger.info(`✅ ${name} test PASSED`);
                }
                else {
                    failed++;
                    results.push({ name, status: 'FAILED', result });
                    logger_1.logger.error(`❌ ${name} test FAILED`);
                }
            }
            catch (error) {
                failed++;
                const errorMessage = error instanceof Error ? error.message : String(error);
                results.push({ name, status: 'ERROR', error: errorMessage });
                logger_1.logger.error(`💥 ${name} test ERROR:`, error);
            }
        }
        logger_1.logger.info(`\n📊 Test Summary: ${passed} passed, ${failed} failed`);
        return { passed, failed, results };
    }
}
exports.IntegrationTest = IntegrationTest;
// 如果直接运行此文件，执行测试
if (require.main === module) {
    const test = new IntegrationTest();
    test.runAllTests().then(summary => {
        process.exit(summary.failed > 0 ? 1 : 0);
    }).catch(error => {
        logger_1.logger.error('Test execution failed:', error);
        process.exit(1);
    });
}
//# sourceMappingURL=integration.test.js.map