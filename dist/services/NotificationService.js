...
class NotificationService {
    ...
    initializeTransporter() {
        ...
        if (config_1.default.email.user && config_1.default.email.password) {
            logger_1.logger.info('Using SMTP for email service');
            return nodemailer_1.default.createTransport({
                host: config_1.default.email.smtp.host,
                port: config_1.default.email.smtp.port,
                secure: config_1.default.email.smtp.port === 465, // true for 465, false for other ports
                auth: {
    ...
    async sendAlert(alert) {
        try {
            ...
            logger_1.logger.info(`Alert sent for ${alert.opportunity.symbol} ${alert.level}`);
            return true;
        }
        catch (error) {
    ...
    generateEmailContent(alert) {
        ...
        const snapshotUrl = `${config_1.default.server.baseUrl}/snapshots/${alert.opportunityId}`;
        return `
    ...
    async sendTestEmail() {
        try {
            const testMailOptions = {
                ...
                to: config_1.default.email.recipients.length > 0 ? config_1.default.email.recipients[0] : 'test@example.com',
        ...
        `
            ...
            logger_1.logger.info('Test email sent successfully', { messageId: info.messageId });
            // 如果是Ethereal测试账户，记录预览URL
            if (info.messageId && info.messageId.includes('ethereal')) {
                const previewUrl = nodemailer_1.default.getTestMessageUrl(info);
                logger_1.logger.info(`Test email preview: ${previewUrl}`);
            }
            return true;
        }
        catch (error) {
            logger_1.logger.error('Failed to send test email:', error);
            return false;
        }
    }
    // 创建Ethereal测试账户
